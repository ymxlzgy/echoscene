...
class BaseInstance3DBoxes:
    ...
    Attributes:
        ...
        box_dim (int): Integer indicating the dimension of a box. Each row is
            (x_size, y_size, z_size, x, y, z, yaw, ...).
    ...
    def __init__(
        ...
        if origin != (0.5, 0.5, 0):
            ...
            self.tensor[:, 3:6] += self.tensor[:, :3] * (dst - src)
    ...
    def volume(self) -> Tensor:
        ...
        return self.tensor[:, 0] * self.tensor[:, 1] * self.tensor[:, 2]
    ...
    def gravity_center(self) -> Tensor:
        ...
        gravity_center[:, 2] = bottom_center[:, 2] + self.tensor[:, 2] * 0.5
    ...
    @property
    def bev(self) -> Tensor:
        """Tensor: 2D BEV box of each box with rotation in WHXYR format, in
        shape (N, 5)."""
    ...
    def in_range_bev(
        ...
        Returns:
            ...
            reference range.
        """
        in_range_flags = ((self.bev[:, 2] > box_range[0])
                          & (self.bev[:, 3] > box_range[1])
                          & (self.bev[:, 2] < box_range[2])
                          & (self.bev[:, 3] < box_range[3]))
    ...
    def overlaps(cls,
        ...
        boxes1_bev[:, 0:2] = boxes1_bev[:, 0:2].clamp(min=1e-4)
        boxes2_bev[:, 0:2] = boxes2_bev[:, 0:2].clamp(min=1e-4)

        # bev overlap
        iou2d = box_iou_rotated(boxes1_bev, boxes2_bev)
        areas1 = (boxes1_bev[:, 0] * boxes1_bev[:, 1]).unsqueeze(1).expand(
            rows, cols)
        areas2 = (boxes2_bev[:, 0] * boxes2_bev[:, 1]).unsqueeze(0).expand(
            rows, cols)
