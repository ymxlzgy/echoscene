...
class BaseInstance3DBoxes:
    ...
    def overlaps(cls,
        ...
        # ``box_iou_rotated``.
        boxes1_bev_, boxes2_bev_ = boxes1.bev, boxes2.bev
        # boxes1_bev[:, 2:4] = boxes1_bev[:, 2:4].clamp(min=1e-4)
        # boxes2_bev[:, 2:4] = boxes2_bev[:, 2:4].clamp(min=1e-4)
        boxes1_bev = boxes1_bev_.clone()
        boxes2_bev = boxes2_bev_.clone()
        boxes1_bev[:, 2:4] = torch.where(boxes1_bev[:, 2:4] < 1e-4, torch.full_like(boxes1_bev[:, 2:4], 1e-4),
                                         boxes1_bev[:, 2:4])
        boxes2_bev[:, 2:4] = torch.where(boxes2_bev[:, 2:4] < 1e-4, torch.full_like(boxes2_bev[:, 2:4], 1e-4),
                                         boxes2_bev[:, 2:4])
