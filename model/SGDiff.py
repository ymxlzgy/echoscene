...
class SGDiff(nn.Module):
    ...
    def sample_box_and_shape(self, dec_objs, dec_triplets, obj_ids, encoded_dec_text_feat, encoded_dec_rel_feat, gen_shape=False):
        if self.type_ == 'cs++_l':
            layout_dict = self.diff.sampleBoxes(dec_objs, dec_triplets, encoded_dec_text_feat, encoded_dec_rel_feat)
            return layout_dict
        elif self.type_ == 'cs++':
            shape_dict, layout_dict = self.diff.sample(dec_objs, dec_triplets, obj_ids, encoded_dec_text_feat, encoded_dec_rel_feat, gen_shape=gen_shape)
    ...
    def sample_boxes_and_shape_with_changes(self, enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat,
                                            dec_objs, dec_triples, obj_ids, encoded_dec_text_feat, encoded_dec_rel_feat, manipulated_nodes, gen_shape=False):
        if self.type_ == 'cs++_l':
            ...
            keep, shape_dict, layout_dict = self.diff.sample_with_changes(enc_objs, enc_triples, encoded_enc_text_feat, encoded_enc_rel_feat,
                                                                    dec_objs, dec_triples, obj_ids, encoded_dec_text_feat, encoded_dec_rel_feat, manipulated_nodes, gen_shape=gen_shape)
