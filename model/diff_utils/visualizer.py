...
class Visualizer():
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.log_dir = os.path.join(opt.hyper.logs_dir, opt.hyper.name)
        else:
            self.log_dir = os.path.join(opt.hyper.results_dir, opt.hyper.name)

        self.img_dir = os.path.join(self.log_dir, 'images')
        self.name = opt.hyper.name
        self.opt = opt
    ...
    def print_current_errors(self, writer, current_iters, errors, t):
        message = f"[{self.opt.hyper.name}] (GPU: {self.opt.hyper.gpu_ids}, iters: {current_iters}, time: {t:.3f}) "
        ...
        self.log_tensorboard_errors(writer, errors, current_iters)

    def print_current_metrics(self, writer, current_iters, metrics, phase):
        ...
        message = f'([{self.opt.hyper.name}] [{phase}] GPU: {self.opt.hyper.gpu_ids}, steps: {current_iters}) '
        ...
        self.log_tensorboard_metrics(writer, metrics, current_iters, phase)

    def display_current_results(self, writer, visuals, current_iters, im_name='', phase='train'):

        # write images to disk
        for label, image_numpy in visuals.items():
            img_path = os.path.join(self.img_dir, f'{phase}_step___{current_iters:05d}_{label}_{im_name}.png')
            util.save_image(image_numpy, img_path)

        # log to tensorboard
        self.log_tensorboard_visuals(writer, visuals, current_iters, phase=phase)

    def log_tensorboard_visuals(self, writer, visuals, cur_step, labels_while_list=None, phase='train'):
    ...
    def log_tensorboard_errors(self, writer, errors, cur_step):

        for label, error in errors.items():
            writer.add_scalar('losses/%s' % label, error, cur_step)

    def log_tensorboard_metrics(self, writer, metrics, cur_step, phase):
